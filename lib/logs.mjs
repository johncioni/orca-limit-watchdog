...
export function appendActivity(dir, text) {
  ...
  // O_NOFOLLOW rejects a symlinked path at open; the post-open fstat then enforces
  // regular/owner/nlink===1 on the actual descriptor. That pair is the security
  // guard, so a separate pre-open assertRegular() lstat is redundant (and would
  // only add a TOCTOU window between the lstat and the open).
