...
export function readLog(file, lines = 100) {
  ...
  if (fd === null) return '';
  try { return lastLines(tailBytes(fd, fs.fstatSync(fd).size).toString('utf8'), lines); }
...
export function maintainLogs(dir) {
  privateDirectory(dir);
  for (const [source, name] of Object.entries(LOG_FILES)) maintainLog(path.join(dir, name), source === 'activity' ? 500 : Infinity);
}
