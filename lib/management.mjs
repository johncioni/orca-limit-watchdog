...
import { readHealth } from './operations.mjs';
...
const { parseStateFile, validateEvent } = fs.existsSync(new URL('../watchdog.mjs', import.meta.url))
...
export function doctorText({ env = process.env } = {}) {
  ...
  if (job?.registered) {
    ...
    loaded = { node: unquote(args?.[0]), runtime: unquote(args?.[1]),
      orca: unquote(job.output.match(/^\s*ORCA_CLI\s*(?:=>|=)\s*(.+)$/m)?.[1]?.trim()) };
  ...
  try {
    if (!fs.existsSync(paths.stateFile)) lines.push('event state: missing (normal before first check)');
      ...
      if (!parseStateFile?.(text)) {
        let why = 'invalid schema';
        try {
          const state = JSON.parse(text);
          if (![1, 2].includes(state?.version)) why = `unsupported version ${state?.version}`;
          else for (const [key, raw] of Object.entries(state.events ?? {})) {
            const error = validateEvent(key, { alertedAt: null, ...raw, ...(state.version === 1 ? { kind: 'limit', platform: 'unknown' } : {}) });
  ...
  } catch (error) { fail(`event state: error (${error.message})`); }
