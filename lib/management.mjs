...
export const LABEL = 'com.john.orca-limit-watchdog';
...
export function installPaths(home = os.homedir()) {
  const stateDir = path.join(home, '.local', 'state', 'orca-limit-watchdog');
  return {
    home,
    stateDir,
    ...
    shareRoot: path.join(home, '.local', 'share', 'orca-limit-watchdog'),
    currentLink: path.join(home, '.local', 'share', 'orca-limit-watchdog', 'current'),
    previousLink: path.join(home, '.local', 'share', 'orca-limit-watchdog', 'previous'),
    commandLink: path.join(home, '.local', 'bin', 'orca-limit-watchdog'),
  };
}
...
export const RELEASE_FILES = Object.freeze([
  ...
  'watchdog.mjs',
  'version.mjs',
  'com.john.orca-limit-watchdog.plist',
  'install.sh',
  'uninstall.sh',
  'README.md',
  'LICENSE',
...
export function validateReleaseRoot(sourceRoot, version, { env = process.env } = {}) {
  ...
  for (const relative of ['watchdog.mjs', 'version.mjs', 'lib/management.mjs', 'bin/orca-limit-watchdog.mjs']) {
    runChecked(process.execPath, ['--check', path.join(sourceRoot, relative)], { env });
  }
  runChecked(plutilPath(env), ['-lint', path.join(sourceRoot, 'com.john.orca-limit-watchdog.plist')], { env });
}

function requireStopped(action, env) {
  if (serviceIsRunning(env)) {
    throw new Error(`run 'orca-limit-watchdog stop' before ${action}; the registered service was not changed`);
  }
}
...
export function installRelease({ sourceRoot, version, env = process.env }) {
  ...
  if (!fs.existsSync(destination)) {
    ...
    try {
      ...
      fs.chmodSync(path.join(staging, 'bin', 'orca-limit-watchdog'), 0o755);
      fs.chmodSync(path.join(staging, 'bin', 'orca-limit-watchdog.mjs'), 0o755);
  ...
  const commandTarget = path.join('..', 'share', 'orca-limit-watchdog', 'current', 'bin', 'orca-limit-watchdog');
  ...
  return `installed ${version}; service is stopped (run 'orca-limit-watchdog start' when ready)`;
}
...
export function uninstallRelease({ env = process.env }) {
  ...
  if (command) {
    const expected = path.join('..', 'share', 'orca-limit-watchdog', 'current', 'bin', 'orca-limit-watchdog');
...
export function serviceIsRunning(env = process.env) {
  ...
  // A spawn/exec error (status null) means the registration state is UNKNOWN, not
  // "absent". Collapsing it to "not running" would fail the requireStopped gate
  // open. Throw so the gate fails closed; a clean nonzero exit is still absent
  // by design (DOG-24).
  if (result.error) throw new Error(`launchctl could not check service state: ${result.error.message}`);
...
export function startService({ env = process.env } = {}) {
  ...
  const paths = installPaths(env.HOME || os.homedir());
  if (serviceIsRunning(env)) {
    const error = new Error(`${LABEL} is already running; run 'orca-limit-watchdog stop' before changing installations`);
    error.exitCode = 1;
    throw error;
  }
...
export function statusText({ env = process.env } = {}) {
  ...
  const paths = installPaths(env.HOME || os.homedir());
  let service;
  try { service = serviceIsRunning(env) ? 'running' : 'stopped'; }
  catch (error) { service = `unknown (${error.message})`; }   // don't crash status on an unverifiable state
  return [
    `service: ${service}`,
    `pause: ${fs.existsSync(paths.disabledFile) ? 'paused' : 'active'}`,
...
export function doctorText({ env = process.env } = {}) {
  ...
  try { lines.push(`launchd: ${serviceIsRunning(env) ? 'running' : 'stopped'}`); }
  catch (error) { lines.push(`launchd: error (${error.message})`); failed = true; }
  const paths = installPaths(env.HOME || os.homedir());
  lines.push(`pause: ${fs.existsSync(paths.disabledFile) ? 'paused' : 'active'}`);
  lines.push(`events: ${eventSummary(paths.stateFile)}`);
...
export const HELP = `Usage: orca-limit-watchdog <command>
...
export async function runCli(args = process.argv.slice(2), { env = process.env, stdout = process.stdout } = {}) {
  ...
  switch (args[0]) {
    case '--version': stdout.write(`orca-limit-watchdog ${VERSION}\n`); return 0;
    case 'doctor': {
