...
```json
{ "version": 1,
  "events": { "<handle>:<sha256(bannerText) [12 hex]>": {
...
- **verify**: if the banner is still in the tail ≥ 10 min after an attempt,
  ...
  loudly).
- **end of life**: an event whose banner is absent from the tail is finished —
  delete it. The same banner text reappearing later is a new event. Events
  whose handle vanishes from `terminal list` (or is stale) are deleted.
...
test/fake-tui.mjs       # prints a real Claude banner, echoes stdin to a file
...
- E2E: run `test/fake-tui.mjs` in a scratch Orca terminal with a resetAt 1 min
